...
os.chdir('/root/bookinfo/')
USERS = [50,1000]

class EntryLoader :
    def __init__(self):
        file = open('data/entries.txt', 'r')
...
class BaseLineLoader :
    def __init__(self):
        file = open('data/entries_baselines.txt', 'r')
        lines = file.readlines()
        self.datas = defaultdict(list)
        for line in lines:
            line = eval(line)
...
for i, user in enumerate(USERS):
    ...
    if value > 3000:
        ax.text(bardefault[0].get_x() + bardefault[0].get_width() / 2, bardefault[0].get_height() , f'{(value/1000):.1f}K ', rotation='vertical', ha='center', va='top', color='white', fontsize=12, fontweight='bold')
    else:
        ax.text(bardefault[0].get_x() + bardefault[0].get_width() / 2, bardefault[0].get_height() , f' {(value/1000):.1f}K', rotation='vertical', ha='center', va='bottom', color='black', fontsize=12, fontweight='bold')
    ...
    if value > 3000:
        ax.text(barNetMarks[0].get_x() + barNetMarks[0].get_width() / 2, barNetMarks[0].get_height() , f'{(value/1000):.1f}K ', rotation='vertical', ha='center', va='top', color='white', fontsize=12, fontweight='bold')
    else:
        ax.text(barNetMarks[0].get_x() + barNetMarks[0].get_width() / 2, barNetMarks[0].get_height() , f' {(value/1000):.1f}K', rotation='vertical', ha='center', va='bottom', color='black', fontsize=12, fontweight='bold')
    ...
    if value > 3000:
        ax.text(barBinPack[0].get_x() + barBinPack[0].get_width() / 2, barBinPack[0].get_height() , f'{(value/1000):.1f}K ', rotation='vertical', ha='center', va='top', color='white', fontsize=12, fontweight='bold')
    else:
        ax.text(barBinPack[0].get_x() + barBinPack[0].get_width() / 2, barBinPack[0].get_height() , f' {(value/1000):.1f}K', rotation='vertical', ha='center', va='bottom', color='black', fontsize=12, fontweight='bold')
    ...
    if sameNetRequests[0] > 3000:
        ax.text(bar[0].get_x() + bar[0].get_width() / 2, bar[0].get_height() , f'{(sameNetRequests[0]/1000):.1f}K ', rotation='vertical', ha='center', va='top', color='white', fontsize=12, fontweight='bold')
    else:
        ax.text(bar[0].get_x() + bar[0].get_width() / 2, bar[0].get_height() , f' {(sameNetRequests[0]/1000):.1f}K', rotation='vertical', ha='center', va='bottom', color='black', fontsize=12, fontweight='bold')
...
for i, baseline in enumerate(baseNames):
    bar = ax2.bar((bar_width+gap_width) * i, baselinesCosts[(baseline)], bar_width, label=baseline, color=palette[-i-1])
    if baselinesCosts[(baseline)] < .51 :
        ax2.text(bar[0].get_x() + bar[0].get_width() / 2, bar[0].get_height() , getCostString(baselinesCosts[(baseline)]) , rotation='vertical', ha='center', va='bottom', color='black', fontsize=13, fontweight='bold')
    else:
        ax2.text(bar[0].get_x() + bar[0].get_width() / 2, bar[0].get_height() , getCostString(baselinesCosts[(baseline)]) , rotation='vertical', ha='center', va='top', color='white', fontsize=13, fontweight='bold')
    ...
    for j, nw in enumerate(net_weights):
        for key, value in costs.items():
            if key[0] == nw and key[2] == cw:
                sameNetRequests.append(value)
    bar = ax2.bar(baselines * (bar_width+gap_width) + index + i * bar_width, sameNetRequests, bar_width, label=f'Cost Weight {cw}', color=palette[i])
    # ax2.text(bar[0].get_x() + bar[0].get_width() / 2, bar[0].get_height() , getCostString(sameNetRequests[0]) , rotation='vertical', ha='center', va='top', color='white', fontsize=13, fontweight='bold')
    if baselinesCosts[(baseline)] < .51 :
        ax2.text(bar[0].get_x() + bar[0].get_width() / 2, bar[0].get_height() , getCostString(sameNetRequests[0]) , rotation='vertical', ha='center', va='bottom', color='black', fontsize=13, fontweight='bold')
    else:
        ax2.text(bar[0].get_x() + bar[0].get_width() / 2, bar[0].get_height() , getCostString(sameNetRequests[0]) , rotation='vertical', ha='center', va='top', color='white', fontsize=13, fontweight='bold')
...
ax2.set_xticklabels(['default', 'NetMARKS', 'BinPacking'] + [f'MOS' for weight in net_weights], fontsize=12)
ax2.tick_params(axis='x')

fig.legend(handles=legend_entries,loc='upper center', bbox_to_anchor=(0.5, -0), ncol=len(legend_entries), fontsize=14)
plt.tight_layout()
plt.savefig(f'thesis_plots/images/Rt_C_Baseline.png', dpi=200, bbox_inches='tight')
