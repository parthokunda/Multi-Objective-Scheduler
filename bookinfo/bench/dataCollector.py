...
def runWebLoad(usercount):
    ...
    os.system(f"locust -f {locustFile} -u {usercount} -r 100 -t {LOADTIME}s -H http://$(kubectl get svc | grep productpage | grep Cluster | awk '{{print $3}}'):9080 --headless --only-summary")
